...
def read_outputs_for_comparison(constant_csv, scenario_csv):
    ...
    for scenario_cfg in scenario_cfgs:
        ...
        output_dir = os.path.join(MAIN_DIR, "studies", study_name, "results", scenario_name)  # construct a path to the result directory for each scenario based on study name and scenario name.
        ...
        return_list.append((tmp_df))  # each DataFrame corresponds to the results of a simulation for a specific scenario.
    return return_list
...
def check_assertions(list_eval_df, all_scenario_assertion_dict):
    ...
    """
    list_result_tuples = []
    for sc_id, assertion_dict in all_scenario_assertion_dict.items():  # key is scenario id and value is dictionary containing assertions for each scenario.
        tmp_df = list_eval_df[sc_id]   # The retrieved DataFrame contains the actual results of the simulation for the scenario.
        scenario_name = tmp_df.loc[G_SCENARIO_NAME, MOD_STR]
        print("-"*80)
        mismatch = False
        for k, v in assertion_dict.items():  # for each scenario, it checks if the actual results('tmp_df') match the expected values(assertions)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        log_level = "info"
        cc = os.path.join(scs_path, "constant_config_energy1.csv")
        sc = os.path.join(scs_path, "example_energy1.csv")
        run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=1)
        list_results = read_outputs_for_comparison(cc, sc)
        all_scenario_assert_dict = {0: {"number users": 88}}  # value is the scenario assertions
